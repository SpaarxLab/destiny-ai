...
- The candidate stores the workspace in the current browser only.
- No account, remote database, analytics, advertising, or silent career-content upload.
- Export is explicit. Import shows a validated preview before replacement.
- Clear is explicit and explains what local data will be removed.
- Browser storage can be cleared or corrupted; the UI must say this plainly and offer export.
...
The product must not guess the participant's location, invent crisis numbers, diagnose risk,
or claim that displaying this notice resolves a safety situation.
