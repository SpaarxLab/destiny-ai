...
export function describeContext(context: PlayerContext): string {
  ...
  if (context.lives.length) {
    parts.push("Lives on the table:\n" + context.lives.map((life) => `- ${life.ref}: "${life.line}" [${life.axis}:${life.pole}]`).join("\n"));
  }
  if (context.picks.stings.length) {
    parts.push("Stings: " + context.picks.stings.map((pick) => `"${pick.line}" (${pick.ref}, tapped ${dwellWord(pick.dwell)})`).join("; "));
    parts.push(context.picks.secret ? `Secret (never admits wanting): "${context.picks.secret.line}" (${context.picks.secret.ref}, tapped ${dwellWord(context.picks.secret.dwell)})` : context.picks.secretSkipped ? "Secret: skipped, too close. Do not mention it." : "");
  }
  if (context.duels.length) {
    parts.push("Duels so far:\n" + context.duels.map((duel) => `- ${duel.reactionRef}: a "${duel.a}" vs b "${duel.b}" (${duel.variable}, ${duel.axis}). You bet ${duel.myBet.pick} for ${duel.myBet.chips} because "${duel.myBet.because}". They picked ${duel.picked} "${duel.pickedLine}" ${dwellWord(duel.dwell)}. ${duel.outcome === "hit" ? "RIGHT" : "WRONG"}${duel.outcome === "miss" && !duel.corrected ? " (not yet explained)" : ""}.`).join("\n"));
  }
  if (context.untested.length) parts.push("Still to duel: " + context.untested.map((life) => `${life.ref} "${life.line}"`).join("; "));
  if (context.coldRead) parts.push(`Your sealed cold read was: "${context.coldRead}"`);
  if (context.lines.length) parts.push("Lines on the table: " + context.lines.map((line) => `${line.kind} "${line.text}" (${line.status})`).join("; "));
  if (context.killed.length) parts.push("KILLED by the person, never repeat or paraphrase: " + context.killed.map((text) => `"${text}"`).join("; "));
  if (context.crowned) parts.push(`Crowned hunger: "${context.crowned}"`);
  if (context.chosenLife) parts.push(`They chose to test: "${context.chosenLife.line}" [${context.chosenLife.axis}:${context.chosenLife.pole}]`);
  if (context.dare) parts.push(`Their dare this week: ${context.dare.action} Done looks like: ${context.dare.doneLooksLike} (${context.dare.days} days, ${context.dare.hours}h, ${context.dare.money} ${context.dare.currency})`);
  return parts.filter(Boolean).join("\n\n");
}

export function movePrompt(move: MoveKind, context: PlayerContext, denial?: string): string {
  const rules = context.rulesOfMe.length ? `\n\nRULES OF ME, in the person's own words. Never break them, never paraphrase around them:\n${context.rulesOfMe.map((rule) => `- ${rule}`).join("\n")}` : "";
  const asked = context.questions.length ? `\n\nYou asked: ${context.questions.map((question) => `"${question.text}" → ${question.answer ? `they said "${question.answer}"` : "no answer yet"}`).join("; ")}.` : "";
  ...
  switch (move) {
    ...
    case "duel": {
      ...
      return `${room}\n\nStage the next DUEL. It must test ${target ? `${target.ref} "${target.line}" [${target.axis}:${target.pole}]` : "the sharpest open question"}: strip one thing off that life and see if it still stings. Side a is pole a of the axis, side b is pole b. Both sides at most nine words, concrete, no titles. Name the variable in at most four words ("money or time"). Then BET: pick a or b, chips 1-3 by how sure you really are, because in at most 80 characters spoken to the person. Scenes: ${SCENES}. Return {"testsLifeRef","axis","variable","a":{"line","scene"},"b":{"line","scene"},"bet":{"pick","chips","because"},"aside"?}.${retry}`;
    }
    case "turn": {
      const allowed = context.allowed;
      const target = context.untested[0];
      const options: string[] = [];
      if (allowed.includes("duel")) options.push(`DUEL: stage the next duel${target ? `, which must test ${target.ref} "${target.line}" [${target.axis}:${target.pole}]` : ", on the sharpest open question"}. Side a is pole a, side b is pole b, both at most nine words, one variable named as "x or y" in at most four words, then BET pick/chips 1-3/because (under 80 chars, to the person). Return {"move":"duel","testsLifeRef","axis","variable","a":{"line","scene"},"b":{"line","scene"},"bet":{"pick","chips","because"},"aside"?}.`);
      ...
      if (allowed.includes("close")) options.push(`CLOSE: stop duelling and go to the verdict now. Do this when you are sure enough or when more duels would only lose chips. You have ${context.record.chips} chips and need 20 to earn the right to describe them; ${context.record.earned ? "you have earned it" : "you have not earned it yet"}. Return {"move":"close","aside"?}.`);
    ...
    case "correction": {
      ...
      return `${room}\n\nYou were WRONG on ${miss?.reactionRef ?? "the last duel"}. Say what you misread in one line, to the person, starting "I misread you." Return {"text": what you thought vs what they did, "correction": the one line}.${retry}`;
    }
    case "verdict": {
